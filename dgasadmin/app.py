...
from dgas.database import prepare_database, create_pool
from dgas.log import configure_logger, log as dgas_log
...
from dgasadmin.utils import process_image

Config.REQUEST_TIMEOUT = 600

from dgas.utils import parse_int

dgas_log.setLevel(logging.DEBUG)
...
def fix_avatar_for_user(id_service_url, user, key='avatar'):
    if key not in user or not user[key]:
        user[key] = "{}/identicon/{}.png".format(id_service_url, user['dgas_id'])
...
async def get_dgas_user_from_payment_address(conf, address):
    ...
    async def check_login(request, *args, **kwargs):
        ...
        if session_cookie:
            async with app.pool.acquire() as con:
                admin = await con.fetchrow("SELECT admins.dgas_id FROM admins "
                                           "JOIN sessions ON admins.dgas_id = sessions.dgas_id "
                                           "WHERE sessions.session_id = $1",
                                           session_cookie)
            if admin:
                url = '{}/v1/user/{}'.format(ID_SERVICE_LOGIN_URL, admin['dgas_id'])
...
async def post_login(request):
    ...
    dgas_id = user['dgas_id']
    session_id = generate_session_id()
    async with app.pool.acquire() as con:
        admin = await con.fetchrow("SELECT * FROM admins WHERE dgas_id = $1", dgas_id)
        if admin:
            await con.execute("INSERT INTO sessions (session_id, dgas_id) VALUES ($1, $2)",
                              session_id, dgas_id)
    if admin:
        ...
        dgas_log.info("Invalid login from: {}".format(dgas_id))
        raise SanicException("Login Failed", status_code=401)
...
async def get_config_home(request, current_user):
    ...
    for admin in admins:
        async with app.configs['mainnet'].db.id.acquire() as con:
            user = await con.fetchrow("SELECT * FROM users WHERE dgas_id = $1", admin['dgas_id'])
        if user is None:
            user = {'dgas_id': admin['dgas_id']}
...
async def post_admin_add_remove(request, current_user, action):
    if 'dgas_id' in request.form:
        dgas_id = request.form.get('dgas_id')
        if not dgas_id:
        ...
        async with app.configs['mainnet'].db.id.acquire() as con:
            ...
            if user is None and username.startswith("0x"):
                user = await con.fetchrow("SELECT * FROM users WHERE dgas_id = $1", username)
        if user is None:
    ...
    if action == 'add':
        print('adding admin: {}'.format(dgas_id))
        async with app.pool.acquire() as con:
            await con.execute("INSERT INTO admins VALUES ($1) ON CONFLICT DO NOTHING", dgas_id)
    elif action == 'remove':
        print('removing admin: {}'.format(dgas_id))
        async with app.pool.acquire() as con:
            await con.execute("DELETE FROM admins WHERE dgas_id = $1", dgas_id)
            await con.execute("DELETE FROM sessions WHERE dgas_id = $1", dgas_id)
    else:
        raise SanicException("Not Found", status_code=404)
...
async def get_txs(request, conf, user):
    ...
    for row in rows:
        tx = dict(row)
        tx['from_user'] = await get_dgas_user_from_payment_address(conf, tx['from_address'])
        tx['to_user'] = await get_dgas_user_from_payment_address(conf, tx['to_address'])
        txs.append(tx)
...
async def get_tx(request, conf, current_user, tx_hash):
    ...
    if 'node' in context or 'db' in context:
        ...
        context['from_user'] = await get_dgas_user_from_payment_address(conf, from_address)
        context['to_user'] = await get_dgas_user_from_payment_address(conf, to_address)
...
@app.route("/user/<dgas_id>", prefixed=True)
@requires_login
async def get_user(request, conf, current_user, dgas_id):
    async with conf.db.id.acquire() as con:
        row = await con.fetchrow(
            "SELECT * FROM users WHERE dgas_id = $1", dgas_id)
    if not row:
    ...
    async with conf.db.rep.acquire() as con:
        reviews_given_rows = await con.fetch(
            "SELECT * FROM reviews WHERE reviewer_id = $1", dgas_id)
        reviews_received_rows = await con.fetch(
            "SELECT * FROM reviews WHERE reviewee_id = $1", dgas_id)
    reviews_given = []
    reviews_received = []
    for review in reviews_given_rows:
        async with conf.db.id.acquire() as con:
            reviewee = await con.fetchrow("SELECT * FROM users WHERE dgas_id = $1", review['reviewee_id'])
        if reviewee:
            reviewee = fix_avatar_for_user(conf.urls.id, dict(reviewee))
        else:
            reviewee = fix_avatar_for_user(conf.urls.id, {'dgas_id': review['reviewee_id']})
    ...
    for review in reviews_received_rows:
        async with conf.db.id.acquire() as con:
            reviewer = await con.fetchrow("SELECT * FROM users WHERE dgas_id = $1", review['reviewer_id'])
        if reviewer:
            reviewer = fix_avatar_for_user(conf.urls.id, dict(reviewer))
        else:
            reviewer = fix_avatar_for_user(conf.urls.id, {'dgas_id': review['reviewer_id']})
    ...
    async with conf.db.id.acquire() as con:
        reports_given_rows = await con.fetch(
            "SELECT users.dgas_id, users.username, users.avatar, reports.details "
            "FROM reports JOIN users "
            "ON reports.reportee_dgas_id = users.dgas_id "
            "WHERE reports.reporter_dgas_id = $1",
            dgas_id)
        reports_received_rows = await con.fetch(
            "SELECT users.dgas_id, users.username, users.avatar, reports.details "
            "FROM reports JOIN users "
            "ON reports.reporter_dgas_id = users.dgas_id "
            "WHERE reports.reportee_dgas_id = $1",
            dgas_id)
...
async def create_dapp(request, conf, current_user):
    ...
    dgas_id = "0x{:040x}".format(dapp_id)
    data, cache_hash, format = process_image(avatar.body, avatar.type)

    async with conf.db.id.acquire() as con:
        await con.execute("INSERT INTO avatars (dgas_id, img, hash, format) VALUES ($1, $2, $3, $4) "
                          "ON CONFLICT (dgas_id, hash) DO UPDATE "
...
async def update_dapp(request, conf, current_user, dapp_id):
    ...
    url = request.form.get('url')
    description = request.form.get('description')
    dgas_id = "0x{:040x}".format(int(dapp_id))
...
async def feature_app_handler_post(request, conf, current_user):
    ...
    if dgas_id is not None:
        async with conf.db.id.acquire() as con:
            await con.execute("UPDATE users SET featured = $2 WHERE dgas_id = $1", dgas_id, True if featured else False)
        if 'Referer' in request.headers:
            return redirect(request.headers['Referer'])
        return redirect("/{}/user/{}".format(conf.name, dgas_id))
    ...
    if dgas_id is not None:
        async with conf.db.id.acquire() as con:
            async with con.transaction():
                await con.execute("UPDATE users SET blocked = $2 WHERE dgas_id = $1", dgas_id, True if blocked else False)
        if 'Referer' in request.headers:
            return redirect(request.headers['Referer'])
        return redirect("/{}/user/{}".format(conf.name, dgas_id))
    return redirect("/{}/apps".format(conf.name))
...
async def get_reports(request, conf, current_user):
    ...
    for row in rows:
        async with conf.db.id.acquire() as con:
            reporter = await con.fetchrow("SELECT * FROM users WHERE dgas_id = $1", row['reporter_dgas_id'])
            reportee = await con.fetchrow("SELECT * FROM users WHERE dgas_id = $1", row['reportee_dgas_id'])
...
async def migrate_users(request, current_user):
    ...
    async with app.configs[from_env].db.id.acquire() as con:
        ...
        for row in user_rows:
            dgas_ids.add(row['dgas_id'])
        avatar_rows = await con.fetch("SELECT * FROM avatars WHERE dgas_id = ANY($1)", dgas_ids)
        users = []
        avatars = []
        for row in user_rows:
    ...
    print("INSERTING {} USERS".format(len(dgas_ids)))
    ...
    async with app.configs[to_env].db.id.acquire() as con:
        rval = await con.executemany(
            ...
            users)
        print("MIGRATED USERS: {} (of {})".format(rval, len(dgas_ids)))
        rval = await con.executemany(
            "INSERT INTO avatars ("
            "dgas_id, img, hash, format, last_modified"
            ") VALUES ($1, $2, $3, $4, $5) "
            "ON CONFLICT DO NOTHING",
            avatars)
