...
if [ ! -d heroku-buildpack-pgsql-stunnel ]; then
    git clone git@github.com:dgasio/heroku-buildpack-pgsql-stunnel.git
fi
...
if [ ! -e .runconfig ]; then
    ...
    heroku config:get DATABASE_URL -a dgas-id-service >> .runconfig
    echo -n "export MAINNET_ETH_SERVICE_DATABASE_URL=" >> .runconfig
    heroku config:get DATABASE_URL -a dgas-eth-service >> .runconfig
    echo -n "export MAINNET_DIR_SERVICE_DATABASE_URL=" >> .runconfig
    heroku config:get DATABASE_URL -a dgas-directory-service >> .runconfig
    echo -n "export MAINNET_REP_SERVICE_DATABASE_URL=" >> .runconfig
    heroku config:get DATABASE_URL -a dgas-reputation-service >> .runconfig
    ...
    heroku config:get ETHEREUM_NODE_URL -a dgas-eth-service >> .runconfig
    ...
    echo "export MAINNET_ID_SERVICE_URL=https://dgas-id-service.herokuapp.com" >> .runconfig
    echo "export MAINNET_ETH_SERVICE_URL=https://dgas-eth-service.herokuapp.com" >> .runconfig
    echo "export MAINNET_DIR_SERVICE_URL=https://dgas-dir-service.herokuapp.com" >> .runconfig
    echo "export MAINNET_REP_SERVICE_URL=https://dgas-rep-service.herokuapp.com" >> .runconfig
...
if [ -z $DATABASE_URL ]; then
    export DATABASE_URL=postgresql://dgas:@127.0.0.1/dgas-admin
fi
...
if [ $# -eq 0 ]; then
    ...
    if command -v entr >/dev/null 2>&1; then
        env/bin/start-stunnel bash -c "find . -iname '*.py' | entr -r env/bin/python -m dgasadmin"
    else
        env/bin/start-stunnel bash -c "env/bin/python -m dgasadmin"
    fi
else
