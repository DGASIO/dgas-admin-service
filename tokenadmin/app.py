...
async def get_user(request, conf, current_user, token_id):
    ...
    if usr['is_app']:
        async with conf.db.dir.acquire() as con:
            row = await con.fetchrow(
                "SELECT * FROM apps JOIN qofp_manifests ON apps.token_id = qofp_manifests.token_id WHERE apps.token_id = $1",
                token_id)
        if row:
            usr['app'] = row
...
async def get_apps(request, conf, current_user):
    ...
    if search_query:
        ...
        sql = ("SELECT * FROM apps "
               "JOIN qofp_manifests ON apps.token_id = qofp_manifests.token_id "
        ...
        async with conf.db.dir.acquire() as con:
            rows = await con.fetch(
                "SELECT * FROM apps JOIN qofp_manifests ON apps.token_id = qofp_manifests.token_id "
    ...
    if token_id is not None:
        async with conf.db.dir.acquire() as con:
            async with con.transaction():
                ...
                await con.execute("DELETE FROM qofp_manifests WHERE token_id = $1", token_id)
